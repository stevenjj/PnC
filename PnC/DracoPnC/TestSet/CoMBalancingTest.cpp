...
CoMBalancingTest::CoMBalancingTest(RobotSystem* robot) : Test(robot) {
    myUtils::pretty_constructor(1, "CoM Balancing Test");
    cfg_ = YAML::LoadFile(THIS_COM"Config/Draco/TEST/COM_BALANCING_TEST.yaml");

    phase_ = CoMBalancingTestPhase::BT_initial_jpos;
    state_list_.clear();
...
CoMBalancingTest::~CoMBalancingTest() {
    for(int i(0); i<state_list_.size(); ++i){
        delete state_list_[i];
    }
}

void CoMBalancingTest::TestInitialization() {
...
int CoMBalancingTest::_NextPhase(const int & phase) {
    int next_phase = phase + 1;
    printf("next phase: %i\n", next_phase);
    if (next_phase == NUM_BT_PHASE) {
        return CoMBalancingTestPhase::BT_com_ctrl;
    }
    else return next_phase;
}

void CoMBalancingTest::_SettingParameter() {
    try {
